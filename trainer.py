...
from dataset import pad_batch, pad_batch_bert, pad_batch_bert_gnn
...
def train(args, train_dataset, model, test_dataset, eval_dataset):
    ...
    collate_fn1 = pad_batch_bert_gnn  # pad a batch
...
def test(args, eval_dataset, model):
    ...
    collate_fn2 = pad_batch_bert_gnn
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            adj, mask, emb, ids, seg, text_len, y = batch
            adj = adj.float().to(args.device)
            mask = mask.float().to(args.device)
            emb = emb.float().to(args.device)
            ids = ids.to(args.device)
